...
def LDALoss(X, Y):
    # X sample 10개 , feature 10개 , class 10개 
    # # 왜냐하면 ipc = 1 , 합성 데이터가 10장이 끝 
    ...
    XList = [X0, X1, X2, X3, X4, X5, X6, X7, X8, X9]

    for c in range(num_classes):

        class_samples = XList[c]
        class_mean = torch.empty(10, 1).to('cuda')

        for i in range (10):
            for j in range (10):
                # print(class_mean[j])   
                # print(class_samples[i])             
                # print(class_samples[i][0, j])

                # print(class_mean[j].shape)
                # print(class_samples[i].shape) 
                # print(class_samples[i][0, j].shape)

                class_mean[j] += class_samples[i][0, j]
        
        # class_mean = class_mean / 10
