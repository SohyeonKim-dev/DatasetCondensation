...
class TensorDataset(Dataset):
    def __init__(self, images, labels): # images: n x c x h x w tensor
        ...
        #  detach() method : gradient의 전파를 멈추는 역할
...
# 위의 정의된 기본 setting에 따라 네트워크를 설정 및 생성
...
# real과 syn의 gradient의 차이를 return?! 
# 이게 우리의 metric 
def distance_wb(gwr, gws):
    ...
    # 논문에 제시된 cos 역수 1 빼기 그 수식 구현한 것 
    dis_weight = torch.sum(1 - torch.sum(gwr * gws, dim=-1) / (torch.norm(gwr, dim=-1) * torch.norm(gws, dim=-1) + 0.000001))
    dis = dis_weight
    return dis


# 각 metric에 맞추어 custom loss (distance) 반환
# default dis_metric -> ours at main.py
def match_loss(gw_syn, gw_real, args):

    # 여기서 distance, 에러, 로스 정의하는 과정인 것 (ours metric, distance_wb을 써서!)
    dis = torch.tensor(0.0).to(args.device)

    if args.dis_metric == 'ours':
        # image 개수에 따라 돌면서 -> distance_wb 호출
        for ig in range(len(gw_real)):
            gwr = gw_real[ig]
            gws = gw_syn[ig]
            dis += distance_wb(gwr, gws)
            # real / syn의 grad를 가져와서, distance를 가져온다
    ...
    # distance_wb, ours로 정의된 error를 return
    return dis


# 우리는 CIFAR 10 -> ipc 10에 해당
def get_loops(ipc):
...
# 중요한 function! 
# epoch method를 통해 train / test loss, acc를 반환 (정해진 Epoch 횟수만큼)
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args):
    ...
    # criterion을 이러한 방식으로 정의 -> CrossEntropyLoss (이게 끝단에 softmax!! 붙는구나. . )
    criterion = nn.CrossEntropyLoss().to(args.device)
    ...
    for ep in range(Epoch+1):
        # epoch을 돌면서 train loss 반환 
        # print("train loader epoch by evaluate synset") # 너무 많이 찍힘
    ...
    # 여기서 정의한 cross entropy가 epoch으로 넘어감!
    # epoch에서 계산된 loss와 accuracy가 반환됨 -> 이걸 print해서 log로 찍어낸 것!
    loss_test, acc_test = epoch('test', testloader, net, optimizer, criterion, args, aug = False)
    print("test loader epoch by evaluate synset") # 40
    # test는 trainable이 아니라서, 여기서 수행하는 것은 더더욱 의미 X
